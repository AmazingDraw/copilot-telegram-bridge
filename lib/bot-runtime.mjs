...
const FIX_CTX_SCRIPT = join(BRIDGE_ROOT, "scripts", "fix-model-tokens.sh");
...
export function attachRuntime(ctx) {
    ...
    // Cancel prior delayed purges so only the latest dismiss schedule runs.
...
async function handleFileAttachment(message) {
    ...
    if (!inboundGate.allowed) {
        ...
        if (inboundGate.reason === "private-denied") {
            await enqueue(() => ctx.sendMessage(chatId, "本 Bot 仅在群组内使用。"));
            return;
        }
        if (inboundGate.reason === "chat-not-allowlisted") {
            await enqueue(() => ctx.sendMessage(chatId, "此群未开通本服务。"));
            return;
        }
        ...
        try {
            ...
            if (prepared) {
                if (!ctx.session) {
                    stopTyping();
                    dismissBubble();
                    await enqueue(() => ctx.sendMessage(chatId, "⚠️ 会话未连接，无法发送附件。请先 /start 或 /telegram connect。"));
                    return;
                }
                // CD：防滥用（按 userId）
                if (ctx.cooldown && typeof ctx.cooldown.check === "function") {
                    const cd = ctx.cooldown.check(userIdStr);
                    if (!cd.ok) {
                        stopTyping();
                        dismissBubble();
                        await enqueue(() =>
                ...
                const promptText = (typeof ctx.stripBotMention === "function"
                    ? ctx.stripBotMention(text || "", botUsername)
                    : text) || "User sent a file. Please reverse-engineer a high-quality image prompt.";
                await ctx.session.send({
                    prompt: promptText,
                    attachments: [prepared.attachment],
                });
                if (ctx.cooldown && typeof ctx.cooldown.touch === "function") ctx.cooldown.touch(userIdStr);
                return;
            }
        } catch (err) {
            stopTyping();
    ...
    if (text) {
        ...
        const blockedCmd = (label) => {
            ...
            dismissBubble();
            return enqueue(() =>
                ctx.sendMessage(chatId, `本 Bot 仅做提示词反推，不支持 ${label}。\n发描述或图片 + @本 Bot 即可。`)
            );
        };
        ...
        // /reboot 命令：重启无头 daemon
        if (trimmedText === "/reboot" || trimmedText.startsWith("/reboot@")) {
            if (restricted) { await blockedCmd("/reboot"); return; }
            stopTyping();
            dismissBubble();
            const daemonScript = join(process.env.HOME || "~", ".copilot/extensions/telegram-bridge/scripts/headless-daemon.sh");
            await enqueue(() => ctx.sendMessage(chatId, "🔄 <b>正在重启无头服务...</b>", "HTML"));
            // 统一用 restart，延迟 1.5s 确保消息发出后再执行
            // （无头侧 restart 会 kill 自身，stop+start 无法在无头进程内回调；桌面侧也兼容）
            setTimeout(() => {
                execFile("bash", [daemonScript, "restart"], { timeout: 30000 }, async (err, _out, stderr) => {
                    if (err && !ctx.connected) return; // 无头被 kill，正常不执行
                    if (err) {
        ...
        // /fixctx 命令：一键修复桌面 App 自定义模型上下文窗口（重置后 128K 问题）
        if (trimmedText === "/fixctx" || trimmedText.startsWith("/fixctx@")) {
            if (restricted) { await blockedCmd("/fixctx"); return; }
            stopTyping();
            dismissBubble();
            const scriptPath = FIX_CTX_SCRIPT;
            await enqueue(() => ctx.sendMessage(chatId, "⏳ 正在修复自定义模型上下文...", "HTML"));
            await new Promise((resolve) => {
                execFile("bash", [scriptPath], { timeout: 15000 }, async (err, stdout, stderr) => {
                    if (err) {
                        await enqueue(() => ctx.sendMessage(chatId,
                            `❌ <b>修复失败</b>\n<pre>${escapeHtml(stderr || err.message)}</pre>`, "HTML"));
                    } else {
                        const lines = stdout.split("\n");
                        const updated = lines.filter(l => l.startsWith("✅")).map(l => {
                            const m = l.match(/\[(.+?)\]/);
                            return m ? `• ${m[1]}` : l;
                        });
                        const skipped = lines.filter(l => l.startsWith("⏭")).map(l => {
                            const m = l.match(/\[(.+?)\]/);
                            return m ? `• ${m[1]}` : l;
                        });
                        const doneLine = lines.find(l => l.startsWith("完成")) || "";
                        let msg = `🔧 <b>上下文修复完成</b>\n`;
                        if (updated.length) msg += `\n✅ <b>已更新</b>\n${escapeHtml(updated.join("\n"))}`;
                        if (skipped.length) msg += `\n\n⏭ <b>已跳过（未添加）</b>\n${escapeHtml(skipped.join("\n"))}`;
                        if (doneLine) msg += `\n\n<i>${escapeHtml(doneLine.replace("完成：", "").trim())}</i>`;
                        msg += `\n\n重开 Copilot session 即生效。`;
                        await enqueue(() => ctx.sendMessage(chatId, msg, "HTML"));
                    }
                    resolve();
                });
            });
        ...
        if (!ctx.session) {
            stopTyping();
            dismissBubble();
            await enqueue(() => ctx.sendMessage(chatId, "⚠️ 会话未连接，无法发送。请先 /start 或 /telegram connect。"));
            return;
        }
