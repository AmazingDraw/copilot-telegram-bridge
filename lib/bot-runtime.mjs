...
async function handleFileAttachment(message) {
    ...
    if (text) {
        ...
        // /new：无头 createSession 开新对话；/start 兼容别名（Telegram 内置按钮仍发 /start）
        if (
            trimmedText === "/new" ||
        ...
        // /fixctx：只更新 ~/.copilot/data.db 的 provider_models 窗口数字，不删会话/provider
        if (trimmedText === "/fixctx" || trimmedText.startsWith("/fixctx@")) {
            ...
            await enqueue(() => ctx.sendMessage(chatId, "⏳ 正在修复模型上下文...", "HTML"));
