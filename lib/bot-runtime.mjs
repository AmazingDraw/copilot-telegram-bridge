...
async function handleFileAttachment(message) {
    ...
    if (text) {
        ...
        // /fixctx 命令：一键修复 Copilot 桌面模型与 OpenCodex catalog 上下文
        if (trimmedText === "/fixctx" || trimmedText.startsWith("/fixctx@")) {
            ...
            await enqueue(() => ctx.sendMessage(chatId, "⏳ 正在修复 Copilot / OpenCodex 模型上下文...", "HTML"));
            await new Promise((resolve) => {
                execFile("bash", [scriptPath], { timeout: 90000 }, async (err, stdout, stderr) => {
                    if (err) {
                        await enqueue(() => ctx.sendMessage(chatId,
                            `❌ <b>修复失败</b>\n<pre>${escapeHtml(stderr || err.message)}</pre>`, "HTML"));
                    } else {
