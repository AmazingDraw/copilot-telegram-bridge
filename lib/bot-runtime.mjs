...
/**
 * Insert a JPEG COM marker before EOI so content sha256 changes.
 * Copilot session dedupes vision by content hash; same TG photo re-sent would
 * otherwise skip binary remount and the model replies with the "not a portrait" stub.
 * Returns true if the file was rewritten.
 */
function bustJpegContentHash(filePath, token) {
    let buf;
    try {
    ...
    const comment = Buffer.from(`pr:${String(token || Date.now())}`.slice(0, 200), "utf8");
    const len = 2 + comment.length;
    ...
    comment.copy(com, 4);
    try {
        writeFileSync(filePath, Buffer.concat([buf.subarray(0, eoi), com, buf.subarray(eoi)]));
        return true;
    } catch {
        return false;
    }
 ...
 */
    ...
    async function rejectIfImageBusyOrCooling(chatId, profile, userIdStr) {
        if (profile?.requireImage && ctx.isAgentBusy) {
            stopTyping();
            dismissBubble();
            // 忙碌拒收文案（requireImage）：改这里。私仓文档「用户可见文案」
...
async function handleFileAttachment(message) {
    ...
    // Unique JPEG COM so repeated identical photos remount for vision
    if (ctx.botProfile?.requireImage) {
        const token = `${message.message_id || 0}-${Date.now()}`;
        if (bustJpegContentHash(localPath, token)) {
            console.error(
                `telegram-bridge: [${ctx.currentBotName || "bot"}] jpeg COM hash-bust message_id=${message.message_id}`
            );
        }
    }
    ...
    if (!inboundGate.allowed) {
        ...
        // 私聊拒绝文案（requireImage / private-denied）：改这里。私仓文档「用户可见文案」
    ...
    if (message.photo || message.document) {
            ...
            if (prepared) {
                ...
                const stripped = (typeof ctx.stripBotMention === "function"
                    ...
                    : text);
                const promptText = ignoreCaption ? (stripped || "Please analyze this image.") : (stripped || "User sent a file.");
    ...
    // 6s（原 8s）：B1 之后心跳只剩本地 RPC（getCurrent/纠偏），6s 足够；调小 = 极端情况少挡一点
    const HEARTBEAT_TIMEOUT_MS = 6_000;
    let lastHeartbeatAt = 0;

    /**
     * 会话心跳（**只允许在收完消息之后调用** —— 它以前排在 getUpdates 之前，
     * 慢/挂时最多阻塞 8s，表现就是"半天没响应，过一会儿一下全回来"；实测 heartbeat timeout 发生 1382 次）。
     * 职责：探测死会话 + 把卡在官方模型上的会话纠偏回来。
     * 为什么不再查官方目录：`isOfficialModelBlocked` 是纯 id/名单判断，不需要云端目录；
     * 而每分钟一次 `model.list()` 在不用官方模型时只会带来 401 和日志噪音（2026-09-13 实测）。
     */
    async function runHeartbeat() {
        if (!ctx.session) return;
        if (Date.now() - lastHeartbeatAt < HEARTBEAT_INTERVAL_MS) return;
        lastHeartbeatAt = Date.now();
        const work = banishBlockedSessionModel(ctx.session, {
        ...
        // 超时后 work 仍可能 reject → 显式吞掉，别变成 unhandled rejection
        work.catch(() => {});
        try {
            await Promise.race([
                work,
    ...
    while (!ctx.shutdownRequested) {
        ...
        try {
            ...
            // A) 心跳在**收完消息之后**：它再慢也挡不住 getUpdates（原先挡在前面，最多阻塞 8s）
            try {
                await runHeartbeat();
            } catch {
                break; // 连接已死（runHeartbeat 内部已置 ctx.connected=false）
            }
        } catch (err) {
