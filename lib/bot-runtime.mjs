 ...
 */
    function reverseCooldownIds(profile, userIdStr) {
        if (profile?.requireImage) return ["__global__", String(userIdStr)];
        return [String(userIdStr)];
    }

    async function rejectIfReverseBusyOrCooling(chatId, profile, userIdStr) {
        if (profile?.requireImage && ctx.isAgentBusy) {
            stopTyping();
            dismissBubble();
            await enqueue(() => ctx.sendMessage(chatId, "⏳ 正在反推，请等这轮结束。"));
            return true;
        }
        if (ctx.cooldown && typeof ctx.cooldown.check === "function") {
            for (const id of reverseCooldownIds(profile, userIdStr)) {
    ...
    function touchReverseCooldown(profile, userIdStr) {
        if (!(ctx.cooldown && typeof ctx.cooldown.touch === "function")) return;
        for (const id of reverseCooldownIds(profile, userIdStr)) ctx.cooldown.touch(id);
    }
    ...
    if (message.photo || message.document) {
        try {
            ...
            if (prepared) {
                ...
                if (await rejectIfReverseBusyOrCooling(chatId, profile, userIdStr)) return;
                ...
                const promptText = ignoreCaption
                    ? "按 PROMPT_REVERSE.md 要求反推这张图，禁止输出模板外任何多余文字（「看不清图」除外）。"
                    : (stripped || "User sent a file. Please reverse-engineer a high-quality image prompt.");
                await ctx.session.send({
                    prompt: promptText,
                    attachments: [prepared.attachment],
                });
                touchReverseCooldown(profile, userIdStr);
                return;
            }
        } catch (err) {
            stopTyping();
