...
import { SESSION_UUID_RE, isSessionResumable } from "./session-fs.mjs";
...
export function createHeadlessLeaderApi(paths) {
    ...
    function resolveHeadlessResumeTarget(botName) {
        const tryId = (sessionId) => {
            if (!sessionId || !isSessionResumable(sessionId)) return null;
            return sessionId;
        };
        try {
            const lock = readLock(botName);
            const hit = tryId(lock?.sessionId);
            if (hit) return hit;
        } catch {}
        try {
            const st = loadJsonOrDefault(botStatePath(botName), {});
            const hit = tryId(st?.lastSessionId);
            if (hit) return hit;
        } catch {}
        return null;
    }
