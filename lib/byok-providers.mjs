...
function normalizeFixctx(raw, id) {
    ...
    const opencodexContextWindow = positiveInt(raw.opencodexContextWindow);
    if (!opencodexContextWindow) {
        throw new Error(`catalog.${id}.fixctx.opencodexContextWindow must be a positive integer`);
    }
    const out = { opencodexContextWindow };
    ...
    if (raw.ensureEnabled === true) out.ensureEnabled = true;
    return out;
}
...
export async function buildHeadlessSessionConfig({
    ...
    const desired = (!allowSet ? defaultModel : null)
        || modelsCfg.officialFallback
        || (officialModels || [])[0]?.id
        || "";
    ...
    if (found || desiredLocal) config.model = found ? found.id : desiredLocal;
