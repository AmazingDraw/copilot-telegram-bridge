...
import { buildHeadlessSessionConfig, loadAgentsMdInstructions, loadModelsConfig, isOfficialModelBlocked } from "./byok-providers.mjs";
import {
    SESSION_STATE_DIR,
    getSessionName,
    setSessionUserName,
...
export function attachCommands(ctx) {
    ...
    async function switchHeadlessSession(chatId, targetSessionId, targetName) {
        ...
        ctx.headlessSwitching = (async () => {
            ...
            // per-bot 锁始终强制；否则仅在目标会话残留官方 auto 等屏蔽模型时纠正，
            // 保留“无锁会话沿用上次模型”的语义。
            if (resumeConfig.model && resumeConfig._forceModelLocal) {
                const perBotLocked = !!(ctx.botProfile?.defaultModel || ctx.botProfile?.allowedModels);
                let shouldForceModel = perBotLocked;
                if (!shouldForceModel) {
                    try {
                        const current = await next.rpc.model.getCurrent();
                        shouldForceModel = isOfficialModelBlocked(current?.modelId, loadModelsConfig());
                    } catch (err) {
                        console.error(`telegram-bridge: [${ctx.currentBotName}] getCurrent failed:`, err.message);
                    }
                }
                if (shouldForceModel) {
                    try {
                        await next.rpc.model.switchTo({
                            modelId: resumeConfig.model,
                            contextTier: "default",
                        });
                        console.error(
                            `telegram-bridge: [${ctx.currentBotName}] forced model → ${resumeConfig.model}`
                        );
                    } catch (swErr) {
                        console.error(
                            `telegram-bridge: [${ctx.currentBotName}] force model switch failed: ${swErr.message}`
                        );
                    }
                }
            }
                ...
                ctx.desktopHandoffTimer = setTimeout(async () => {
                    ...
                    try {
                        ...
                        if (checkLock && checkLock.sessionId === handoffTarget && checkLock.pid === 0) {
                            ...
                            // pollLoop 见锁切走后已 connected=false 退出；写回自有锁后须立刻重连，
                            // 否则 startLockPoller 不认领 pid===self，桥会假死。
                            if (!ctx.connected && typeof ctx.handleConnect === "function") {
                                console.error(
                                    `telegram-bridge: handoff timeout; reconnecting previous session ${handoffPrev}`
                                );
                                try {
                                    await ctx.handleConnect(handoffBot, handoffPrev);
                                } catch (reconErr) {
                                    console.error(
                                        `telegram-bridge: handoff timeout reconnect failed:`,
                                        reconErr.message
                                    );
                                }
                            }
