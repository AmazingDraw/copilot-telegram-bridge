...
import { loadBotCliproxyApiKey } from "./bot-profile.mjs";
import {
    SESSION_STATE_DIR,
    getSessionName,
    setSessionUserName,
...
export function attachCommands(ctx) {
    ...
    async function buildLiveHeadlessConfig({ defaultModel = null, forceDefaultModel = null } = {}) {
        const agentsMd = (typeof ctx.loadAgentsFromPath === "function") ? ctx.loadAgentsFromPath(ctx.botProfile?.agentsMd, () => loadAgentsMdInstructions()) : loadAgentsMdInstructions(ctx.botProfile?.agentsMd);
        let officialModels = [];
        if (!officialModelsEnabled()) {
            console.error(`telegram-bridge: [${ctx.currentBotName}] 官方模型面已关闭 → 跳过 listModels`);
        } else try {
            if (typeof ctx.headlessClient?.listModels === "function") {
                officialModels = await ctx.headlessClient.listModels();
            }
        } catch (err) {
            console.error(`telegram-bridge: [${ctx.currentBotName}] listModels 失败（已继续，BYOK 不受影响）：${compactError(err)}`);
        }
        ...
        const config = await buildHeadlessSessionConfig({
            ...
            disabledMcpServers: ctx.botProfile?.disabledMcpServers || [],
            clientMode: ctx.botProfile?.clientMode || "copilot-cli",
            availableTools: ctx.botProfile?.availableTools || null,
            ...
            cliproxyApiKey: loadBotCliproxyApiKey(ctx.botProfile),
            providerId: ctx.botProfile?.providerId || null,
        });
    ...
    async function adoptHeadlessSession(next, prevSession, prevSid, fallbackSid) {
        ...
        if (typeof ctx.logSessionSkills === "function") {
            await ctx.logSessionSkills(next);
        } else {
            console.error("telegram-bridge: warn: ctx.logSessionSkills 未接线，技能面自证缺失");
        }
        ctx.session = next;
    ...
    function getSessionTokenUsage(sessionId) {
        ...
        // 内存是 compact 后的活值（0 从不写入）。不要 Math.max，否则压完百分比粘在高峰。
    ...
    async function handleStatusCommand(chatId) {
        try {
            ...
            try {
                ...
                // 现查成功（含 compact 后变小）以 SDK 为准；0/空才回落缓存。
                if (usedTokens == null) {
                    usedTokens = fallback;
                }
