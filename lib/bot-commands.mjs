...
export function attachCommands(ctx) {
    ...
    async function buildLiveHeadlessConfig() {
    ...
    async function startHeadlessNewSession() {
        if (!ctx.isHeadless) {
            throw new Error("桌面 join 模式无法从 Telegram 开新对话，请用无头 Bot，或在 Copilot App 里新建会话。");
        }
        if (!ctx.headlessClient) {
            throw new Error("无头客户端未就绪，请稍后重试");
        }
        ...
        ctx.headlessSwitching = (async () => {
            ...
            console.error(`telegram-bridge: [${ctx.currentBotName}] headless createSession (new)`);
    ...
    async function handleNewCommand(chatId) {
        try {
            ...
            await ctx.enqueue(() => ctx.sendMessage(
                chatId,
                `✅ <b>已开启新对话</b>\n会话：<code>${escapeHtml(short || "unknown")}</code>\n模型已避开官方 auto。可直接发消息。`,
                "HTML"
            ));
        } catch (err) {
