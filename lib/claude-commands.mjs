...
function loadBridgeConfig() {
    ...
    try {
        ...
        return {
            claude: {
                agentsDir: paths.claudeAgentsDir || `${home}/.codex`,
                sessionDir: paths.claudeSessionDir || `${home}/.claude/sessions`,
                stateDir: paths.claudeStateDir || "/tmp/cu-card/claude",
    ...
    return {
        claude: {
            agentsDir: `${home}/.codex`,
            sessionDir: `${home}/.claude/sessions`,
            stateDir: "/tmp/cu-card/claude",
...
export function attachClaudeCommands(ctx) {
    ...
    const CLAUDE_AGENTS_DIR = bridgeCfg.claude.agentsDir;
    const CLAUDE_SESSION_DIR = bridgeCfg.claude.sessionDir;
    ...
    function launchClaudeTask(chatId, task, prompt, imagePath = "") {
        return new Promise((resolve) => {
            ...
            const workDir = CLAUDE_AGENTS_DIR;
            ...
            const cmdArgs = ["-p", effectivePrompt, "--dangerously-skip-permissions"];
            if (ctx.claudeModel) {
                cmdArgs.push("--model", ctx.claudeModel);
            }
    ...
    async function handleClaudeResumeMenu(chatId) {
        try {
            ...
            for (const t of Object.values(tasks)) {
                ...
                if (!seen.has(t.sessionId)) {
                    seen.set(t.sessionId, {
                        sid: t.sessionId,
                        title: t.prompt || `会话 ${t.sessionId.slice(0, 8)}`,
                        time: t.startedAt || 0,
                    });
                }
            }
            ...
            if (existsSync(CLAUDE_SESSION_DIR)) {
                try {
                    const files = readdirSync(CLAUDE_SESSION_DIR).filter((f) => f.endsWith(".json"));
                    for (const f of files) {
                        const sid = f.replace(/\.json$/, "");
                        if (seen.has(sid)) continue;
                        const p = join(CLAUDE_SESSION_DIR, f);
            ...
            const sorted = [...seen.values()].sort((a, b) => b.time - a.time).slice(0, 10);
            const buttons = [];
            for (const item of sorted) {
                const sidHash = getClaudeSidHash(item.sid);
                claudeTitleCache.set(item.sid, item.title);
                buttons.push([{ text: truncateVisual(item.title, CELL_W - 4), callback_data: `claude:resume:${sidHash}` }]);
            }
    ...
    async function handleClaudeModel(chatId) {
        const current = ctx.claudeModel || "cliproxy/claude-sonnet-4-6";
        let models = [];
        try {
    ...
    async function tryConsumeClaudeInput(chatId, text, messageId, imagePath = "") {
        ...
        const task = { taskId: claudeTaskId(), mode: pending.mode, sessionId: pending.sessionId };
        if (task.sessionId) {
            const cur = loadClaudeTasks();
