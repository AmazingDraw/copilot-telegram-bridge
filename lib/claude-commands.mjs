...
// 通过 CLI Proxy (:8317) 的 Anthropic /v1/messages 驱动 Claude Code（不经 OpenCodex）。
...
import { loadModelsConfig, listConfiguredCliModelSlugs, parseCliproxyYaml } from "./byok-providers.mjs";
...
function buildClaudeSystemPrompt(workDir, claudeDir) {
    const cwdLine = `CWD: ${workDir}`;
    const persona = loadClaudePersonaText(claudeDir);
    return persona ? `${cwdLine}\n\n${persona}` : cwdLine;
}
...
/** Claude Code → cliproxy Anthropic 端点（不读 OpenCodex claude-env.sh）。 */
...
export function attachClaudeCommands(ctx) {
    ...
    function launchClaudeTask(chatId, task, prompt, imagePath = "") {
        return new Promise((resolve) => {
            ...
            // CWD 顶掉默认 Date；--bare 不自动发现 CLAUDE.md，因此把 ~/.claude/CLAUDE.md 显式拼进 system prompt
            cmdArgs.push("--system-prompt", buildClaudeSystemPrompt(workDir, CLAUDE_DIR));
