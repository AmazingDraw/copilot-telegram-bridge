...
// 通过 CLI Proxy (:8317) 的 Anthropic /v1/messages 驱动 Claude Code。
...
import { BRIDGE_ROOT, loadModelsConfig, listConfiguredCliModelSlugs, parseCliproxyYaml, resolveConfigPath } from "./byok-providers.mjs";

/**
 * 解析 Claude Code 可执行文件路径。
 * 优先级：
 * 1. 环境变量 CLAUDE_BIN
 * 2. 配置文件 config/models.json -> paths.claudeBin
 * 3. Cherry Studio 内置的原生 Claude Code 引擎（无需系统单独安装）
 * 4. 系统 PATH 里的 claude
 */
export function resolveClaudeBinPath(customPath = "") {
    if (process.env.CLAUDE_BIN && existsSync(process.env.CLAUDE_BIN)) {
        return process.env.CLAUDE_BIN;
    }
    const raw = customPath || loadModelsConfig()?.paths?.claudeBin;
    if (raw) {
        const expanded = resolveConfigPath(raw);
        if (existsSync(expanded)) return expanded;
    }
    const cherryCandidates = [
        "/Applications/Cherry Studio.app/Contents/Resources/app.asar.unpacked/node_modules/@anthropic-ai/claude-agent-sdk-darwin-arm64/claude",
        "/Applications/Cherry Studio.app/Contents/Resources/app.asar.unpacked/node_modules/@anthropic-ai/claude-agent-sdk-darwin-x64/claude",
    ];
    for (const p of cherryCandidates) {
...
function loadBridgeConfig() {
    ...
    try {
        ...
        const prefix = Object.prototype.hasOwnProperty.call(defaults, "claudeModelPrefix")
            ? String(defaults.claudeModelPrefix || "").trim()
            : "";
        const toSlug = (id) => {
        ...
        return {
            claude: {
                bin: paths.claudeBin || "",
                personaPath: paths.claudePersona || "",
                ...
                models,
                modelPrefix: prefix,
                haikuModel: toSlug(defaults.claudeHaikuModel || ""),
                smallFastModel: toSlug(defaults.claudeSmallFastModel || ""),
                bare: defaults.claudeBare !== false,
                strictMcp: defaults.claudeStrictMcp !== false,
            },
...
function loadClaudePersonaText(configuredPersonaPath) {
    const candidates = [
        configuredPersonaPath,
        join(BRIDGE_ROOT, "memory", "CLAUDE.md"),
...
function stripOpenAiV1(url) {
    let u = String(url || "").trim().replace(/\/+$/, "");
    if (u.endsWith("/v1")) u = u.slice(0, -3).replace(/\/+$/, "");
    return u;
}

function resolveCliproxyAnthropicBase() {
    const fromEnv = process.env.CLIPROXY_BASE_URL || process.env.CLI_PROXY_BASE_URL || "";
    const stripped = stripOpenAiV1(fromEnv);
    ...
    try {
        ...
        let host = "127.0.0.1";
        let port = 8317;
        const providerUrl = (cfg.providers || []).find((p) => p.id === "cliproxy")?.baseUrl || "";
        if (providerUrl) {
            try {
                const u = new URL(providerUrl.includes("://") ? providerUrl : `http://${providerUrl}`);
...
function resolveCliproxyApiKey() {
    const fromEnv = process.env.CLIPROXY_API_KEY || process.env.CLI_PROXY_API_KEY || "";
    if (fromEnv) return fromEnv;
    try {
        const yamlPath = loadModelsConfig().paths?.cliproxyConfig;
        if (yamlPath && existsSync(yamlPath)) {
            const parsed = parseCliproxyYaml(readFileSync(yamlPath, "utf8"));
...
function toBareClaudeModel(id) {
    let s = String(id || "").trim();
    if (s.startsWith("cliproxy/")) s = s.slice("cliproxy/".length);
    return s;
}

/** Claude Code → cliproxy Anthropic 端点环境变量配置。 */
function getCliproxyClaudeEnv() {
    let haiku = "";
    let smallFast = "";
    try {
        const d = loadModelsConfig().defaults || {};
        haiku = toBareClaudeModel(d.claudeHaikuModel);
        smallFast = toBareClaudeModel(d.claudeSmallFastModel);
    } catch (_) {}
    const env = {
        ...process.env,
        ANTHROPIC_BASE_URL: resolveCliproxyAnthropicBase(),
        CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY: "1",
        CLAUDE_CODE_MAX_RETRIES: "1",
    };
    delete env.CLAUDE_CODE_RETRY_WATCHDOG;
    const token = resolveCliproxyApiKey();
...
export function attachClaudeCommands(ctx) {
    ...
    const CLAUDE_TIMEOUT_MS = 10 * 60 * 1000;      // 无输出超过 10 分钟才杀
    const CLAUDE_STUCK_MS = 5 * 60 * 1000;         // 5 分钟无新输出判定卡住
    const CLAUDE_LIVE_EDIT_MS = 2400;              // 实时进度 editMessage 节流
    ...
    function isClaudeRateLimited(diag, stdout, stderr) {
        if (diag?.reason && /429|速率限制|配额/.test(diag.reason)) return true;
        const blob = `${stdout || ""}\n${stderr || ""}`;
        return /429|rate.?limit|quota|RESOURCE_EXHAUSTED|cooling down|model_cooldown/i.test(blob);
    }
    ...
    function listClaudeModelSlugs() {
        const configured = listConfiguredCliModelSlugs();
    ...
    function parseClaudeError({ code, stdout, stderr, timedOut, killed, mode, sessionId }) {
        ...
        if (timedOut) {
            ...
            reason = "🔌 本地 CLI Proxy 未启动（cliproxy :8317 拒绝连接）";
            suggestion = "请确认 CLI Proxy (:8317) 服务已正常运行。";
            ...
            suggestion = "请检查 ~/.cli-proxy-api/ 中的认证凭据。";
    ...
    function launchClaudeTask(chatId, task, prompt, imagePath = "") {
        return new Promise((resolve) => {
            ...
            const selectedModel = String(task.forceModel || ctx.claudeModel || defaultModel).trim();
            const effort = resolvedClaudeEffort();
            const fallback = task.usedBridgeFallback ? "" : resolvedClaudeFallback();
            ...
            if (bridgeCfg.claude.bare !== false) cmdArgs.push("--bare");
            if (bridgeCfg.claude.strictMcp !== false) cmdArgs.push("--strict-mcp-config");
            // --bare 不自动发现 CLAUDE.md；append 到默认 system prompt，不覆盖官方 Date / CWD
            const persona = loadClaudePersonaText(bridgeCfg.claude.personaPath);
            if (persona) cmdArgs.push("--append-system-prompt", persona);
            ...
            const customEnv = getCliproxyClaudeEnv();
            const claudeBin = resolveClaudeBinPath(bridgeCfg.claude.bin);
            console.error(
                `telegram-bridge: [${chatId}] /claude spawn bin=${claudeBin} model=${selectedModel} taskId=${taskId}`
            );
            ...
            const bumpProgress = () => {
                ...
                if (Date.now() - lastEditAt < CLAUDE_LIVE_EDIT_MS) return;
                lastEditAt = Date.now();
                editInFlight = true;
            ...
            const capIo = (buf, add, max = 1_500_000) => {
                const s2 = buf + add;
                return s2.length > max ? s2.slice(-max) : s2;
            };
            child.stdout.on("data", (d) => {
                lastOutputAt = Date.now();
                const chunk = d.toString();
                stdoutBuf = capIo(stdoutBuf, chunk);
                consumeClaudeStreamJson(chunk, streamState);
                bumpProgress();
            });
            child.stderr.on("data", (d) => {
                lastOutputAt = Date.now();
                stderrBuf = capIo(stderrBuf, d.toString(), 400_000);
            });

            const timeoutCheck = setInterval(() => {
                if (child.exitCode === null && Date.now() - lastOutputAt > CLAUDE_TIMEOUT_MS) {
            ...
            child.on("close", (code) => {
                ...
                const canBridgeFallback = failCode
                    && !wasCancelled
                    && !task.usedBridgeFallback
                    && fallback
                    && fallback !== selectedModel
                    && isClaudeRateLimited(diag, stdoutBuf, stderrBuf + (streamState.streamError || ""));

                saveClaudeTasks(cur);
                claudeRunningPids.delete(taskId);

                if (canBridgeFallback) {
                    ctx.enqueue(async () => {
                        try {
                            if (progressMsgId) {
                                try {
                                    await ctx.editMessageText(
                                        chatId,
                                        progressMsgId,
                                        `🛟 <b>主模型 429</b>，改用 <code>${escapeHtml(fallback)}</code>`,
                                        "HTML"
                                    );
                                } catch (_) {}
                            } else {
                                await ctx.sendMessage(
                                    chatId,
                                    `🛟 主模型 <code>${escapeHtml(selectedModel)}</code> 配额耗尽，改用 <code>${escapeHtml(fallback)}</code>`,
                                    "HTML"
                                );
                            }
                        } catch (_) {}
                    ...
                    launchClaudeTask(chatId, nextTask, prompt, imagePath).then(resolve);
                    return;
                }

                ctx.enqueue(async () => {
                    try {
                        if (progressMsgId && (failCode || wasCancelled)) {
                            try {
                                await ctx.editMessageText(
                                    chatId,
                                    progressMsgId,
                                    failCode ? "❌ <b>Claude 结束（失败）</b>" : "✋ <b>Claude 已停止</b>",
                                    "HTML"
                                );
                            } catch (_) {}
                        }
                        if (!failCode && !wasCancelled) {
                            ...
                            if (progressMsgId && typeof ctx.deleteMessage === "function") {
                                try { await ctx.deleteMessage(chatId, progressMsgId); } catch (_) {}
                            }
    ...
    function drainClaudePendingQueue() {
        ...
        if (!next) return;
        if (sessionOrChildBusy(next.task?.sessionId || "")) {
            claudePendingQueue.unshift(next);
            return;
        }
        ctx.enqueue(async () => {
            if (next.messageId) {
    ...
    /** 停止运行中的 Claude 任务 */
    function killClaudeChild(child) {
    ...
    function waitForClaudeChildExit(child, ms = 2500) {
        return new Promise((resolve) => {
    ...
    /** 杀掉运行中/卡住的 child，等到进程退出。不改排队。 */
    async function interruptClaudeRunning() {
        const tasks = loadClaudeTasks();
        const live = [];
        for (const [id, t] of Object.entries(tasks)) {
            if (t.status === "running" || t.status === "stuck") {
                t.taskId = t.taskId || id;
                t.status = "cancelled";
                t.finishedAt = Date.now();
                t.interrupted = true;
            }
        ...
        saveClaudeTasks(tasks);
        for (const [id, child] of [...claudeRunningPids.entries()]) {
            if (child && child.exitCode === null) {
                try { child.kill("SIGTERM"); } catch (_) {}
                live.push(waitForClaudeChildExit(child));
            }
        }
        if (live.length) await Promise.all(live);
    }

    function sessionOrChildBusy(sessionId = "") {
        for (const child of claudeRunningPids.values()) {
            if (child && child.exitCode === null) return true;
        }
        const cur = loadClaudeTasks();
        return Object.values(cur).some((t) => {
            const live = t.status === "running" || t.status === "stuck";
    ...
    async function stopRunningClaudeTask(chatId) {
        const tasks = loadClaudeTasks();
        const running = Object.entries(tasks).filter(([, t]) => t.status === "running" || t.status === "stuck");
        let stoppedCount = 0;
        for (const [id, t] of running) {
            const child = claudeRunningPids.get(t.taskId || id);
            if (killClaudeChild(child)) stoppedCount++;
            t.taskId = t.taskId || id;
            t.status = "cancelled";
            t.finishedAt = Date.now();
        }
        ...
        saveClaudeTasks(tasks);
        if (stoppedCount === 0 && running.length === 0) {
    ...
    function busyChoiceKeyboard() {
        return {
            inline_keyboard: [[
                { text: "⏳ 排队", callback_data: "claude:busy:queue" },
                { text: "⚡️ 打断", callback_data: "claude:busy:interrupt" },
                { text: "🗑 取消", callback_data: "claude:busy:cancel" },
            ]],
        };
    }

    async function offerClaudeBusyChoice(chatId, task, prompt, imagePath, messageId) {
        ctx.claudeBusyOffer = { chatId: Number(chatId), task, prompt, imagePath, messageId, queuedAt: Date.now() };
        await ctx.enqueue(() =>
            ctx.sendMessageWithKeyboard(
                chatId,
                `⏳ <b>正在执行任务</b>\n\n新指令：<i>${escapeHtml(String(prompt).slice(0, 80))}</i>\n\n排队 = 结束后再跑 · 打断 = 停掉当前立刻跑这条 · 取消 = 丢掉这条`,
                busyChoiceKeyboard(),
                "HTML"
            )
        ).catch(() => {});
    ...
    async function resolveClaudeBusyChoice(chatId, action) {
        const offer = ctx.claudeBusyOffer;
        ctx.claudeBusyOffer = null;
        if (!offer || Number(offer.chatId) !== Number(chatId)) {
            await ctx.enqueue(() => ctx.sendMessage(chatId, "⚪ 没有待处理的新指令。", "HTML"));
            return;
        }
        if (action === "cancel") {
            await ctx.enqueue(() => ctx.sendMessage(chatId, "🗑 已取消这条指令。", "HTML"));
            return;
        }
        if (action === "queue") {
            claudePendingQueue.push(offer);
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, `⏳ <b>已排队</b>，当前任务结束后执行：<i>${escapeHtml(String(offer.prompt).slice(0, 60))}</i>`, "HTML")
            );
            return;
        }
        claudePendingQueue.length = 0;
        if (sessionOrChildBusy("")) await interruptClaudeRunning();
        if (offer.messageId) ctx.setMessageReaction(offer.chatId, offer.messageId, "🤔").catch(() => {});
        await ctx.enqueue(() =>
            ctx.sendMessage(chatId, `⚡️ <b>已打断</b>，立刻执行：<i>${escapeHtml(String(offer.prompt).slice(0, 60))}</i>`, "HTML")
        ).catch(() => {});
        launchClaudeTask(offer.chatId, offer.task, offer.prompt, offer.imagePath);
    }

    async function interruptAndRunLatest(chatId) {
        const latest = claudePendingQueue.length ? claudePendingQueue.pop() : null;
        claudePendingQueue.length = 0;
        const hadLive = sessionOrChildBusy("");
        if (hadLive) await interruptClaudeRunning();
        if (!latest) {
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, hadLive
                    ? "⚡️ <b>已打断</b>，没有排队指令可接。再发一条就会开跑。"
                    : "⚪ 当前没有运行中的任务，也没有排队。", "HTML")
            );
            return;
        }
        if (latest.messageId) {
            ctx.setMessageReaction(latest.chatId, latest.messageId, "🤔").catch(() => {});
        }
        await ctx.enqueue(() =>
            ctx.sendMessage(chatId, `⚡️ <b>已打断</b>，立刻执行：<i>${escapeHtml(String(latest.prompt).slice(0, 60))}</i>`, "HTML")
        ).catch(() => {});
        launchClaudeTask(latest.chatId, latest.task, latest.prompt, latest.imagePath);
    }
    ...
    function collectClaudeSessions(limit = 20) {
        const tasks = loadClaudeTasks();
        const seen = new Map();
            ...
            try {
                ...
                for (const line of histLines) {
                    ...
                    try {
                        ...
                        const ts = tsRaw > 0 && tsRaw < 1e12 ? tsRaw * 1000 : tsRaw;
                        if (sid) {
                            const prev = seen.get(sid) || { sid, title: "", time: 0, paths: [] };
                            if (disp && !disp.startsWith("/") && !prev.title) prev.title = disp;
    ...
    function deleteClaudeSessionFiles(sid) {
        let removed = 0;
        if (existsSync(CLAUDE_PROJECTS_DIR)) {
            try {
                for (const pDir of readdirSync(CLAUDE_PROJECTS_DIR)) {
                    const fullPath = join(CLAUDE_PROJECTS_DIR, pDir, `${sid}.jsonl`);
                    if (existsSync(fullPath)) {
                        try { unlinkSync(fullPath); removed += 1; } catch (_) {}
                    }
                }
            } catch (_) {}
        }
        if (existsSync(CLAUDE_HIST_FILE)) {
            try {
                const kept = readFileSync(CLAUDE_HIST_FILE, "utf-8").split("\n").filter((line) => {
                ...
                writeFileSync(CLAUDE_HIST_FILE, kept.length ? kept.join("\n") + "\n" : "");
            } catch (_) {}
        }
        const tasks = loadClaudeTasks();
        let dirty = false;
        ...
        if (dirty) saveClaudeTasks(tasks);
        if (ctx.awaitingClaude?.sessionId === sid) clearAwaitingClaude();
        claudeTitleCache.delete(sid);
        return removed;
    }

    async function handleClaudeManageMenu(chatId) {
        const CELL_W = 50;
        const sorted = collectClaudeSessions(15);
        const buttons = [];
        for (const item of sorted) {
            ...
            buttons.push([{ text: truncateVisual(`🗑 ${displayTitle}`, CELL_W - 4), callback_data: `claude:delask:${sidHash}` }]);
        }
        const nums = ["①", "②", "③", "④", "⑤", "⑥", "⑦", "⑧", "⑨", "⑩", "⑪", "⑫", "⑬", "⑭", "⑮"];
        ...
        if (!buttons.length) {
            buttons.push([{ text: "（无可删除会话）", callback_data: "claude:none" }]);
        }
        await ctx.enqueue(() =>
            ctx.sendMessageWithKeyboard(chatId,
                "🗂 <b>管理 Claude 对话</b>\n\n点一条即可删除该会话文件（需确认）。不影响 settings。",
                { inline_keyboard: buttons }, "HTML")
        );
    }
    ...
    async function handleClaudeResumeMenu(chatId) {
        try {
            const CELL_W = 50;
            const sorted = collectClaudeSessions(10);
            const buttons = [];
            for (const item of sorted) {
                const sidHash = getClaudeSidHash(item.sid);
                const displayTitle = item.title || `会话 ${item.sid.slice(0, 8)}`;
    ...
    async function handleClaudeCommand(chatId, argText = "") {
        if (argText && argText.trim() && !argText.trim().startsWith("/")) {
            ...
            if (sessionOrChildBusy("")) {
                await offerClaudeBusyChoice(chatId, task, prompt, "", 0);
                return;
            }
            launchClaudeTask(chatId, task, prompt);
            return;
        }
        ...
        const keyboard = {
            inline_keyboard: [
                ...
                [
                    ...
                    { text: "⚡️ 思考档位", callback_data: "claude:effort" },
                ...
                [
                    ...
                    { text: "🗂 管理对话", callback_data: "claude:manage" },
                ],
                [
    ...
    async function handleClaudeCallback(callbackId, data, chatId) {
        ...
        if (data === "claude:new") {
        ...
        } else if (data === "claude:interrupt") {
            await ctx.answerCallbackQuery(callbackId, "打断并执行最新排队...").catch(() => {});
            await interruptAndRunLatest(chatId);
        } else if (data === "claude:busy:queue") {
            await ctx.answerCallbackQuery(callbackId, "已排队").catch(() => {});
            await resolveClaudeBusyChoice(chatId, "queue");
        } else if (data === "claude:busy:interrupt") {
            await ctx.answerCallbackQuery(callbackId, "打断").catch(() => {});
            await resolveClaudeBusyChoice(chatId, "interrupt");
        } else if (data === "claude:busy:cancel") {
            await ctx.answerCallbackQuery(callbackId, "已取消").catch(() => {});
            await resolveClaudeBusyChoice(chatId, "cancel");
        } else if (data === "claude:progress") {
            await ctx.answerCallbackQuery(callbackId, "查看进度").catch(() => {});
            await handleClaudeProgress(chatId);
        } else if (data === "claude:manage") {
            await ctx.answerCallbackQuery(callbackId, "管理对话").catch(() => {});
            await handleClaudeManageMenu(chatId);
        } else if (data.startsWith("claude:delask:")) {
            const sid = resolveClaudeSid(data.slice("claude:delask:".length));
            if (!sid) {
                await ctx.answerCallbackQuery(callbackId, "会话已失效，请重新 /claude").catch(() => {});
                return;
            }
            await ctx.answerCallbackQuery(callbackId, "确认删除").catch(() => {});
            const title = resolveClaudeTitle(sid) || sid.slice(0, 10);
            const h = getClaudeSidHash(sid);
            await ctx.enqueue(() =>
                ctx.sendMessageWithKeyboard(chatId,
                    `🗑 <b>删除 Claude 对话？</b>\n\n会话：<b>${escapeHtml(title)}</b>\n\n将删除对应 jsonl，并从 history 去掉记录。不可恢复。`,
                    { inline_keyboard: [[
                        { text: "✅ 确认删除", callback_data: `claude:del:${h}` },
                        { text: "↩️ 取消", callback_data: "claude:manage" },
                    ]] }, "HTML")
            );
        } else if (data.startsWith("claude:del:")) {
            const sid = resolveClaudeSid(data.slice("claude:del:".length));
            if (!sid) {
                await ctx.answerCallbackQuery(callbackId, "会话已失效").catch(() => {});
                return;
            }
            const title = resolveClaudeTitle(sid) || sid.slice(0, 10);
            const n = deleteClaudeSessionFiles(sid);
            await ctx.answerCallbackQuery(callbackId, "已删除").catch(() => {});
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, `🗑 已删除 Claude 对话 <b>${escapeHtml(title)}</b>（${n} 个会话文件）。`, "HTML")
            );
            await handleClaudeManageMenu(chatId);
            ...
            await ctx.answerCallbackQuery(callbackId, "思考档位").catch(() => {});
            ...
            await ctx.enqueue(() => ctx.sendMessage(chatId, "⚡️ 思考档位已恢复为 CLI 默认。", "HTML"));
            ...
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, `⚡️ <b>思考档位</b>：${em.icon} ${em.label}（<code>${id}</code>）\n仅当前对话生效。`, "HTML")
            );
    ...
    async function tryConsumeClaudeInput(chatId, text, messageId, imagePath = "") {
        ...
        if (sessionOrChildBusy(task.sessionId)) {
            await offerClaudeBusyChoice(chatId, task, raw || "请分析这张图片。", imagePath, messageId);
            return true;
        }
