...
export function normalizeModelHtmlBreaks(md) {
    ...
    t = t.replace(/\[[^\]\n]+\]\([^)\n]+\)/g, (m) => hold(m.replace(/<br\s*\/?>/gi, " ")));
    t = t.replace(/<br\s*\/?>/gi, "\n");
    t = t.replace(/\x05(\d+)\x05/g, (_, i) => holds[parseInt(i, 10)]);
    return t;
}
...
export function tableToHtmlTable({ headers, rows }) {
    const headerCells = headers.map(h => `<td><b>${escapeInline(h)}</b></td>`).join("");
    const bodyRows = rows.map(row => {
        const cells = row.map(c => `<td>${escapeInline(c || "—")}</td>`).join("");
        return `<tr>${cells}</tr>`;
    }).join("");
...
export function tableToList({ headers, rows }) {
    ...
    return rows.map((row, rIdx) => {
        ...
        const firstVal = escapeInline(row[0] || "—");

        // 单列/简单列表
        if (headers.length === 1) {
            return `• <b>${firstHeader}</b>: ${firstVal}`;
        }

        // 两列键值对卡片
        if (headers.length === 2) {
            const secondHeader = escapeInline(headers[1] || "内容");
            const secondVal = escapeInline(row[1] || "—");
            return `📌 <b>${firstVal}</b> (${firstHeader})\n   └ 🔹 <b>${secondHeader}</b>: ${secondVal}`;
        }
        ...
        for (let i = 1; i < headers.length; i++) {
            ...
            const v = escapeInline(row[i] || "—");
            const isLast = (i === headers.length - 1);
            const prefix = isLast ? "└ 🔸" : "├ 🔹";
            subItems.push(`   ${prefix} <b>${h}</b>: ${v}`);
        }
...
export function normalizeLooseMarkdownTables(md) {
    ...
    t = out.join("\n");
    t = t.replace(/\x05(\d+)\x05/g, (_, idx) => holds[parseInt(idx, 10)]);
    return t;
}
...
function cleanUnsupportedHtmlTagsAfterEscape(html) {
    ...
    t = t.replace(/\x06(\d+)\x06/g, (_, i) => holds[parseInt(i, 10)]);
...
export function escapeInline(text) {
    ...
    // 注意：必须【先还原 linkHolds (\x02)】，将链接内可能嵌套的 \x01 展开放回字符串中，【再还原 codeHolds (\x01)】，防止 \x01 残留被 Telegram 渲染为 "0" 错误超链接
    t = t.replace(/\x02(\d+)\x02/g, (_, i) => linkHolds[parseInt(i)]);
    t = t.replace(/\x01(\d+)\x01/g, (_, i) => codeHolds[parseInt(i)]);
...
export function richConvertTextPart(text) {
    ...
    result = result.replace(/\x00(\d+)\x00/g, (_, idx) => holds[parseInt(idx)]);
