...
import { loadModelsConfig, listConfiguredCliModelSlugs } from "./byok-providers.mjs";
...
export function attachCodexCommands(ctx) {
    ...
    async function handleCodexModel(chatId) {
        ...
        let models = listConfiguredCliModelSlugs();
        try {
            ...
            const filtered = models.filter((s) => !disabled.has(s));
            if (filtered.length) models = filtered;
        } catch (_) {}
        if (!models.length) {
            await ctx.enqueue(() => ctx.sendMessage(chatId, "⚠️ 未配置 CLI 模型列表。请在 <code>config/models.json</code> 的 <code>modelSets.claude-cli</code> 增删。", "HTML"));
            return;
        }
        const rows = [];
