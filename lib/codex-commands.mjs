...
function listJsonlByMtime(root) {
    const out = [];
    const walk = (dir) => {
        let ents;
        try { ents = readdirSync(dir, { withFileTypes: true }); } catch (_) { return; }
        for (const e of ents) {
            const fp = join(dir, e.name);
            if (e.isDirectory()) walk(fp);
            else if (e.isFile() && e.name.endsWith(".jsonl")) {
                try { out.push({ path: fp, mtime: statSync(fp).mtimeMs }); } catch (_) {}
            }
        }
    };
...
export function attachCodexCommands(ctx) {
    ...
    const CODEX_TIMEOUT_MS = 10 * 60 * 1000;      // 无输出超过 10 分钟才杀
    const CODEX_STUCK_MS = 5 * 60 * 1000;         // 5 分钟无新输出判定卡住
    const CODEX_SLIM_DISABLE_PLUGINS = [
    ...
    function launchCodexTask(chatId, task, prompt, imagePath = "") {
        return new Promise((resolve) => {
            ...
            const tasksRec = tasks[taskId];
            if (tasksRec) tasksRec.taskId = taskId;

            const basePrompt = `${prompt}`;
            const modelArg = ctx.codexModel ? ["-m", ctx.codexModel] : [];
            const imageArg = imagePath ? ["-i", imagePath] : [];
            const slimArg = [];
            if (bridgeCfg.codex.slim !== false) {
                slimArg.push("--disable", "memories", "--ignore-rules");
                slimArg.push("-c", "features.memories=false");
                slimArg.push("-c", "mcp_servers.node_repl.enabled=false");
                slimArg.push("-c", "mcp_servers.computer-use.enabled=false");
                slimArg.push("-c", "mcp_servers.cua_repl.enabled=false");
                for (const id of CODEX_SLIM_DISABLE_PLUGINS) {
                    slimArg.push("-c", `plugins."${id}".enabled=false`);
                }
            }
            let cmdArgs;
            if (task.mode === "resume" && task.sessionId) {
                cmdArgs = ["exec", "resume", task.sessionId, basePrompt, ...modelArg, ...imageArg, "-o", doneFile, "--skip-git-repo-check", ...slimArg];
            } else {
                cmdArgs = ["exec", basePrompt, ...modelArg, ...imageArg, "-o", doneFile, "--skip-git-repo-check", ...slimArg];
            }
            ...
            child.stdout.on("data", (d) => {
                ...
                stdoutBuf = capIo(stdoutBuf, d.toString());
            });
            ...
            const timeoutCheck = setInterval(() => {
                if (child.exitCode === null && Date.now() - lastOutputAt > CODEX_TIMEOUT_MS) {
                ...
                if (code === 0 && (!task.sessionId || task.mode === "new")) {
                    try {
                        if (existsSync(CODEX_SESSION_DIR)) {
                            const files = listJsonlByMtime(CODEX_SESSION_DIR);
                            if (files.length > 0) {
                                const newestFile = files[0].path;
    ...
    function sessionOrChildBusy(sessionId = "") {
        for (const child of codexRunningPids.values()) {
    ...
    async function stopRunningCodexTask(chatId) {
        const cur = loadCodexTasks();
        const running = Object.entries(cur).filter(([, t]) => t.status === "running" || t.status === "stuck");
        if (running.length === 0) {
    ...
    function busyChoiceKeyboard() {
        return {
            inline_keyboard: [[
                { text: "⏳ 排队", callback_data: "codex:busy:queue" },
                { text: "⚡️ 打断", callback_data: "codex:busy:interrupt" },
                { text: "🗑 取消", callback_data: "codex:busy:cancel" },
            ]],
        };
    }

    async function offerCodexBusyChoice(chatId, task, prompt, imagePath, messageId) {
        ctx.codexBusyOffer = { chatId: Number(chatId), task, prompt, imagePath, messageId, queuedAt: Date.now() };
        await ctx.enqueue(() =>
    ...
    async function resolveCodexBusyChoice(chatId, action) {
        const offer = ctx.codexBusyOffer;
        ctx.codexBusyOffer = null;
        ...
        codexPendingQueue.length = 0;
        if (sessionOrChildBusy("")) await interruptCodexRunning();
        ...
        launchCodexTask(offer.chatId, offer.task, offer.prompt, offer.imagePath);
    }

    function waitForCodexChildExit(child, ms = 5000) {
        return new Promise((resolve) => {
    ...
    async function interruptCodexRunning() {
        const cur = loadCodexTasks();
        ...
        for (const child of [...codexRunningPids.values()]) {
            if (child && child.exitCode === null) {
                try { child.kill("SIGTERM"); } catch (_) {}
                waits.push(waitForCodexChildExit(child));
            }
        }
        if (waits.length) await Promise.all(waits);
    }

    async function interruptAndRunLatestCodex(chatId) {
        const latest = codexPendingQueue.length ? codexPendingQueue.pop() : null;
        codexPendingQueue.length = 0;
        const hadLive = sessionOrChildBusy("");
        if (hadLive) await interruptCodexRunning();
        if (!latest) {
            await ctx.enqueue(() =>
        ...
        launchCodexTask(latest.chatId, latest.task, latest.prompt, latest.imagePath);
    }
    ...
    function collectCodexSessions(limit = 20) {
        let titles = {};
        try {
            const gs = JSON.parse(readFileSync(join(process.env.HOME || "~", ".codex/.codex-global-state.json"), "utf-8"));
            titles = gs?.["electron-persisted-atom-state"]?.["thread-descriptions-v1"] || {};
        } catch (_) {}
        const files = listJsonlByMtime(CODEX_SESSION_DIR);
        const seen = new Map();
        for (const { path: f } of files) {
            try {
                ...
                const title = extractCodexSessionTitle(lines, titles, sid, threadId);
                seen.set(sid, { sid, title, path: f, threadId });
    ...
    function deleteCodexSessionFiles(sid) {
        let removed = 0;
        for (const { path: f } of listJsonlByMtime(CODEX_SESSION_DIR)) {
            try {
        ...
        if (dirty) saveCodexTasks(tasks);
        if (ctx.awaitingCodex?.sessionId === sid) clearAwaitingCodex();
        codexTitleCache.delete(sid);
        return removed;
    }

    async function handleCodexManageMenu(chatId) {
        try {
            const CELL_W = 50;
            const items = collectCodexSessions(15);
            const buttons = [];
            for (const item of items) {
                const sidHash = getCodexSidHash(item.sid);
                const label = item.title || item.sid.slice(0, 10);
                codexTitleCache.set(item.sid, item.title || "");
                buttons.push([{ text: truncateVisual(`🗑 ${label}`, CELL_W - 4), callback_data: `codex:delask:${sidHash}` }]);
            }
            const nums = ["①", "②", "③", "④", "⑤", "⑥", "⑦", "⑧", "⑨", "⑩", "⑪", "⑫", "⑬", "⑭", "⑮"];
            ...
            if (!buttons.length) {
                buttons.push([{ text: "（无可删除会话）", callback_data: "codex:none" }]);
            }
            await ctx.enqueue(() =>
            ...
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, `❌ 列出会话失败: ${escapeHtml(err.message)}`, "HTML")
            );
        }
    }
    ...
    async function handleCodexResumeMenu(chatId) {
        try {
            const CELL_W = 50;
            const items = collectCodexSessions(10);
            const buttons = [];
            for (const item of items) {
                try {
                    const sid = item.sid;
    ...
    async function handleCodexCommand(chatId, argText = "") {
        ...
        if (argText && argText.trim() && !argText.trim().startsWith("/")) {
            ...
            if (sessionOrChildBusy("")) {
                await offerCodexBusyChoice(chatId, task, prompt, "", 0);
                return;
            }
            launchCodexTask(chatId, task, prompt);
            return;
        }
        // 主菜单（六子命令）
        const keyboard = {
            inline_keyboard: [
                ...
                [
                    { text: "🗂 管理对话", callback_data: "codex:manage" },
                    { text: "🖥 关闭桌面", callback_data: "codex:desktop" },
                ],
                [
    ...
    async function handleCodexCallback(callbackId, data, chatId) {
        ...
        } else if (data === "codex:interrupt") {
            await ctx.answerCallbackQuery(callbackId, "打断并执行最新排队...").catch(() => {});
            await interruptAndRunLatestCodex(chatId);
        } else if (data === "codex:busy:queue") {
            await ctx.answerCallbackQuery(callbackId, "已排队").catch(() => {});
            await resolveCodexBusyChoice(chatId, "queue");
        } else if (data === "codex:busy:interrupt") {
            await ctx.answerCallbackQuery(callbackId, "打断").catch(() => {});
            await resolveCodexBusyChoice(chatId, "interrupt");
        } else if (data === "codex:busy:cancel") {
            await ctx.answerCallbackQuery(callbackId, "已取消").catch(() => {});
            await resolveCodexBusyChoice(chatId, "cancel");
        } else if (data === "codex:progress") {
            await ctx.answerCallbackQuery(callbackId, "查看进度").catch(() => {});
            await handleCodexProgress(chatId);
        } else if (data === "codex:manage") {
            await ctx.answerCallbackQuery(callbackId, "管理对话").catch(() => {});
            await handleCodexManageMenu(chatId);
        } else if (data === "codex:none") {
            await ctx.answerCallbackQuery(callbackId, "无会话").catch(() => {});
        } else if (data.startsWith("codex:delask:")) {
            const sid = resolveCodexSid(data.slice("codex:delask:".length));
            if (!sid) {
                await ctx.answerCallbackQuery(callbackId, "会话已失效，请重新 /codex").catch(() => {});
                return;
            }
            await ctx.answerCallbackQuery(callbackId, "确认删除").catch(() => {});
            const title = resolveCodexTitle(sid) || sid.slice(0, 10);
            const h = getCodexSidHash(sid);
            await ctx.enqueue(() =>
                ctx.sendMessageWithKeyboard(chatId,
                    `🗑 <b>删除 Codex 对话？</b>\n\n会话：<b>${escapeHtml(title)}</b>\n\n将删除对应 session jsonl。不可恢复。`,
                    { inline_keyboard: [[
                        { text: "✅ 确认删除", callback_data: `codex:del:${h}` },
                        { text: "↩️ 取消", callback_data: "codex:manage" },
                    ]] }, "HTML")
            );
        } else if (data.startsWith("codex:del:")) {
            const sid = resolveCodexSid(data.slice("codex:del:".length));
            if (!sid) {
                await ctx.answerCallbackQuery(callbackId, "会话已失效").catch(() => {});
                return;
            }
            const title = resolveCodexTitle(sid) || sid.slice(0, 10);
            const n = deleteCodexSessionFiles(sid);
            await ctx.answerCallbackQuery(callbackId, "已删除").catch(() => {});
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, `🗑 已删除 Codex 对话 <b>${escapeHtml(title)}</b>（${n} 个会话文件）。`, "HTML")
            );
            await handleCodexManageMenu(chatId);
    ...
    async function tryConsumeCodexInput(chatId, text, messageId, imagePath = "") {
        ...
        if (sessionOrChildBusy(task.sessionId)) {
            await offerCodexBusyChoice(chatId, task, raw || "请分析这张图片。", imagePath, messageId);
            return true;
        }
