...
function loadBridgeConfig() {
    ...
    try {
        ...
        return {
            codex: {
                agentsDir: paths.codexAgentsDir || `${home}/.gemini/config/plugins/amazing-draw-plugin/skills/amazing-draw`,
    ...
    return {
        codex: {
            agentsDir: `${home}/.gemini/config/plugins/amazing-draw-plugin/skills/amazing-draw`,
...
export function attachCodexCommands(ctx) {
    ...
    async function handleCodexCommand(chatId, argText = "") {
        if (!existsSync(CODEX_AGENTS_DIR)) {
            await ctx.enqueue(() =>
                ctx.sendMessage(chatId, "⚠️ amazing-draw 目录不存在，无法确定 workdir。仍可尝试运行。", "HTML")
            );
        }
