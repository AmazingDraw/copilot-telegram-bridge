...
export function attachHandlers(ctx) {
    ...
    function setupEventHandlers(sess) {
        ...
        function recordSessionTokens(tokens) {
        ...
        function tokensFromEventData(data) {
            if (!data || typeof data !== "object") return null;
            for (const k of ["currentTokens", "totalTokens", "tokensAfter"]) {
                const v = data[k];
                if (typeof v === "number" && Number.isFinite(v) && v > 0) return v;
            }
            const used = data.contextWindow?.used ?? data.usage?.currentTokens;
            if (typeof used === "number" && Number.isFinite(used) && used > 0) return used;
            return null;
        }

        // 实时上下文占用；忽略 0。compact 后会变小，必须写入，不能只升不降。
        sess.on("session.usage_info", (event) => {
            recordSessionTokens(tokensFromEventData(event?.data) ?? event?.data?.currentTokens);
        });

        sess.on("session.compaction_complete", (event) => {
            const data = event?.data || event;
            recordSessionTokens(tokensFromEventData(data));
        });
    ...
    /**
     * 会话级权限姿态（allow-all / deny-all）。仅 permissionMode 非 ask 时调用。
     *
     * 版本差异（实测 vendored SDK）：
     *   - ≤1.0.80：`permissions.setAllowAll({enabled})` = tool + path + URL 一并放行
     *   - ≥1.0.83：`setAllowAll` **已被移除**，替代品是 `setMode({mode})`，
     *     `PermissionMode = "manual" | "assisted" | "allow-all"`，
     *     而 `allow-all` 的官方注释就是「Tool, path, and URL permission requests are automatically
     *     approved」→ 与旧 setAllowAll **同域**，故 allow-all→"allow-all"、deny-all→"manual"。
     *   - `setApproveAll` 两版都在，保留作「再保险」：显式开关 tool 请求自动批准。
     *   - `setMode` 是 @experimental，且会返回**权威的** post-mutation mode → 用它自证，不盲信调用成功。
     */
    async function applySessionPermissionMode(sess, allowAll) {
        const perms = sess?.rpc?.permissions;
        if (!perms) return;
        const label = allowAll ? "allow-all" : "deny-all";

        if (typeof perms.setMode === "function") {
            // 1.0.83+
            const want = allowAll ? "allow-all" : "manual";
            // 先读当前模式：既能覆盖「resume 后到 setMode 之间的时间窗」，也让日志自证前后态
            let before = "?";
            if (typeof perms.getMode === "function") {
                try {
                    before = (await perms.getMode())?.mode ?? "?";
                } catch (err) {
                    before = `err(${err.message})`;
                }
            }
            try {
                const res = await perms.setMode({ mode: want, source: "rpc" });
                const got = res?.mode ?? "?";
                const ok = res?.success !== false && got === want;
                console.error(
                    `telegram-bridge: [${ctx.name}] permissions.setMode(${want}) ${ok ? "ok" : "⚠️ 未生效"}` +
                    ` ${before}→${got} success=${res?.success}`
                );
            } catch (err) {
                console.error(`telegram-bridge: [${ctx.name}] setMode(${want}) failed: ${err.message}`);
            }
        } else if (typeof perms.setAllowAll === "function") {
            // 老运行时（≤1.0.80）兼容
            try {
                await perms.setAllowAll({ enabled: allowAll, source: "rpc" });
                console.error(`telegram-bridge: [${ctx.name}] permissions.setAllowAll(${allowAll}) ${label}`);
            } catch (err) {
                console.error(`telegram-bridge: [${ctx.name}] setAllowAll(${allowAll}) failed: ${err.message}`);
            }
        } else {
            console.error(
                `telegram-bridge: [${ctx.name}] permissions: setMode 与 setAllowAll 都不存在（SDK 又变了？）`
            );
        }

        try {
            await perms.setApproveAll({ enabled: allowAll, source: "rpc" });
            console.error(`telegram-bridge: [${ctx.name}] permissions.setApproveAll(${allowAll}) ${label}`);
        } catch (err) {
            console.error(`telegram-bridge: [${ctx.name}] setApproveAll(${allowAll}) failed: ${err.message}`);
        }
    }
    ...
    async function enableHeadlessAllowAll(sess) {
        ...
        // 专 bot（deny-all）：不放行工具；其余（无头缺省 / allow-all）一路绿灯
        await applySessionPermissionMode(sess, mode !== "deny-all");
    }

    /**
     * 技能面自证：读回本会话**实际**可用的技能（含 enabled 状态）。
     * 为什么必须读回来：`enableSkills`/`skillDirectories`/`includedBuiltinSkills` 只是我们的**意图**，
     * 运行时的发现层可能额外塞东西（SDK 技能解析内部是 `enableConfigDiscovery ?? true`）。
     * 只有 skills.list 的返回值才算数 —— 让"关没关干净"可验证，而不是靠猜。
     */
    async function logSessionSkills(sess) {
        if (typeof sess?.rpc?.skills?.list !== "function") {
            console.error(`telegram-bridge: [${ctx.name}] skills.list 不可用（SDK 变了？）`);
            return;
        }
        try {
            const res = await sess.rpc.skills.list();
            const all = Array.isArray(res?.skills) ? res.skills : [];
            const on = all.filter((s) => s?.enabled !== false);
            const names = on.map((s) => String(s?.name || "?")).filter(Boolean);
            const sample = names.slice(0, 10).join(", ");
            console.error(
                `telegram-bridge: [${ctx.name}] skills 自证：可用 ${on.length}/${all.length}` +
                (names.length ? ` → ${sample}${names.length > 10 ? ` …(+${names.length - 10})` : ""}` : "（无）")
            );
        } catch (err) {
            console.error(`telegram-bridge: [${ctx.name}] skills.list failed: ${err.message}`);
        }
    }
    ...
    ctx.logSessionSkills = logSessionSkills;
