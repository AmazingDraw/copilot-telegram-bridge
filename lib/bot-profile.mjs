...
export function resolveBotProfile(name, bot = {}, enabledIndex = 0) {
    const role =
        bot.role === "editor" || bot.role === "headless"
            ? bot.role
            : enabledIndex === 0
              ? "editor"
              : "headless";
    ...
    const defaultModel = bot.defaultModel ? String(bot.defaultModel).trim() : null;
    const allowedModels = Array.isArray(bot.allowedModels)
        ? bot.allowedModels.map((x) => String(x).trim()).filter(Boolean)
        : (defaultModel ? [defaultModel] : null);
    // 若只写了 defaultModel 且 profile 明确单模型场景，允许 only 列表；显式空数组 = 不限制
    const modelAllowlist =
        Array.isArray(bot.allowedModels) && bot.allowedModels.length === 0
            ? null
            : allowedModels;
