...
/**
 * editor = joinSession 桌面 bot；headless = create/resume。
 * 显式 `role` 优先。缺省时：名称 Copilot/Editor → editor；
 * Headless 或已有 profile → headless；其它仍按启用序第 1 个 = editor。
 * 这样关掉 Copilot 后，名为 Headless 的 bot 不会被误当成 joinSession。
 * @param {string} name
...
export function inferBotRole(name, bot = {}, enabledIndex = 0) {
    if (bot.role === "editor" || bot.role === "headless") return bot.role;
    const key = String(name || "").trim().toLowerCase();
    if (key === "copilot" || key === "editor") return "editor";
    if (key === "headless" || bot.profile) return "headless";
    return enabledIndex === 0 ? "editor" : "headless";
}
...
export function resolveBotProfile(name, bot = {}, enabledIndex = 0) {
    ...
    const permissionMode = String(bot.permissionMode || (isHeadless ? "allow-all" : "ask")).toLowerCase();
    ...
    if (Array.isArray(bot.skillNames) && bot.skillNames.length) {
        ...
        const skillSetKey = bot.skillSet != null && String(bot.skillSet).trim() !== ""
            ...
            : (isHeadless && loadSkills ? "headless" : "");
        if (skillSetKey === "all") {
            skillNames = null;
...
export function evaluateInboundAccess({ profile, userId, message, isOwnerAllowed, botUsername }) {
    ...
    if (chatType === "group" || chatType === "supergroup") {
        if (profile.allowedChats.length > 0 && !profile.allowedChats.includes(Number(chatId))) {
            return { allowed: false, reason: "chat-not-allowlisted", chatId, chatType };
        }
            ...
            if (uname) {
                for (const ent of entities) {
                    if (ent.type === "mention") {
                        ...
                        if (slice === `@${uname}`) mentioned = true;
                    }
                    if (ent.type === "text_mention" && ent.user?.is_bot) {
                        // bot text_mention — accept if same id unavailable; soft accept
                        mentioned = true;
                    }
                }
                // slash commands /start@Bot also count
                if (new RegExp(`^/\\w+@${uname}\\b`, "i").test(text.trim())) mentioned = true;
            }
            // reply to bot message
            const reply = message.reply_to_message;
            if (reply?.from?.is_bot) mentioned = true;
            // photos/docs without caption: allow without mention (media reverse use-case)
            if (!text.trim() && (message.photo || message.document)) mentioned = true;
            if (!mentioned) {
