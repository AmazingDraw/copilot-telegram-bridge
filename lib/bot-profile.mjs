...
export function evaluateInboundAccess({ profile, userId, message, isOwnerAllowed, botUsername }) {
    ...
    if (chatType === "private") {
        if (profile.denyPrivate) {
            // 仅 /vip* 与深链 /start vip 放行。裸 /start（点开 Bot 自动发）走 private-denied，不进模型。
            const vip = /^\/(vip|vip_status|vip_grant)(?:@\w+)?(?:\s|$)/i.test(String(text || "").trim())
                || /^\/start(?:@\w+)?(?:\s+vip)(?:\s|$)/i.test(String(text || "").trim());
            if (vip) {
                return { allowed: true, reason: "private-vip", chatId, chatType };
            }
