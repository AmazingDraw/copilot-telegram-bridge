...
export function resolveBotProfile(name, bot = {}, enabledIndex = 0) {
    ...
    const dailyLimitRaw = Number(bot.dailyLimit);
    const dailyLimit = Number.isFinite(dailyLimitRaw) && dailyLimitRaw > 0
        ? Math.max(1, Math.floor(dailyLimitRaw))
        // Free-tier fallback; must match VIP constants FREE_DAILY (VIP folder is OSS-dropped).
        : 0;
    const dailyLimitTz = String(bot.dailyLimitTz || "Asia/Shanghai").trim() || "Asia/Shanghai";
    const rulesInjectEveryRaw = Number(bot.rulesInjectEvery);
    const rulesInjectEvery = Number.isFinite(rulesInjectEveryRaw) && rulesInjectEveryRaw > 0
        ? Math.max(1, Math.floor(rulesInjectEveryRaw))
        : 0;
    // 白名单群备注（只写在配置/文档/日志，不发给 Telegram）：
    //   -1002055502030 测试群
    //   -1004305878659 正式群
    ...
    const requireImage = bot.requireImage === true;
    const restrictedCommands = bot.restrictedCommands !== false && (
        permissionMode === "deny-all" ||
        accessMode === "open-group"
    );
    let commandsMenu = undefined;
    if (Array.isArray(bot.commandsMenu)) {
        commandsMenu = bot.commandsMenu;
    }
    ...
    // 本 Bot 的模型组声明了 provider ⇒ 只走这台上游（切换上游＝改 models.json 里那一个词 + restart）
    const providerId = setPolicy?.provider ? String(setPolicy.provider).trim() : null;
    ...
    // MCP：显式 loadMcp 优先；deny-all 默认不加载（工具反正 deny）
    let loadMcp;
    if (bot.loadMcp === true) loadMcp = true;
    else if (bot.loadMcp === false) loadMcp = false;
    else loadMcp = !(permissionMode === "deny-all");
    // Skills：显式 loadSkills 优先；deny-all 默认不加载技能（避免无用 33 个技能元数据污染）
    let loadSkills;
    if (bot.loadSkills === true) loadSkills = true;
    else if (bot.loadSkills === false) loadSkills = false;
    else loadSkills = !(permissionMode === "deny-all");
    // 官方 client 模式：`empty` = 可选特性默认全关 + 工具过滤 deny-wins（SDK 明示多用户服务应用它）。
    // bots.json 不写 = 老行为（copilot-cli）；改一行即可回滚。
    const clientMode = bot.clientMode === "empty" ? "empty" : "copilot-cli";
    // empty 模式硬要求：每个 session 必须显式声明工具面（SDK 校验不过会直接 throw）。
    // 正常 bot 显式全开（builtin/mcp/custom 三个来源通配）；deny-all 的专 bot 一个工具都不给。
    const availableTools = clientMode === "empty"
        ? (permissionMode === "deny-all" ? [] : ["builtin:*", "mcp:*", "custom:*"])
        : null;
    // MCP 硬禁用名单（bots.json 显式给；与 loadMcp 推导的名单在 resolveDisabledMcpServers 里合并）
    const disabledMcpServers = Array.isArray(bot.disabledMcpServers)
        ? bot.disabledMcpServers.map((n) => String(n || "").trim()).filter(Boolean)
        : [];
    ...
    const cliproxyKeyRaw = String(bot.cliproxyApiKeyFromFile || setPolicy?.cliproxyApiKeyFromFile || "").trim();
    const cliproxyApiKeyFromFile = cliproxyKeyRaw ? expandHomePath(cliproxyKeyRaw) : null;
    if (setPolicy?.requireCliproxyKeyFile && !cliproxyApiKeyFromFile) {
        throw new Error(`bot '${name}' modelSet requires cliproxyApiKeyFromFile`);
    }
    ...
    return {
        ...
        mcpServerNames,
        skillNames,
        cliproxyApiKeyFromFile,
        raw: bot,
    };
}

/**
 * Load a per-bot cliproxy API key from a 600 file.
 * If a path is configured, a missing or empty file is fatal (no fallback).
 * @param {ReturnType<typeof resolveBotProfile>} profile
 * @returns {string|null}
 */
export function loadBotCliproxyApiKey(profile) {
    const path = profile?.cliproxyApiKeyFromFile || null;
    if (!path) {
        return null;
    }
    if (!existsSync(path)) {
        throw new Error(`cliproxy API key file missing: ${path}`);
    }
    const key = readFileSync(path, "utf8").trim();
    if (!key) {
        throw new Error(`cliproxy API key file empty: ${path}`);
    }
    return key;
}
...
/**
 * Per-user daily quota (calendar day in timeZone). File: bots/<name>/daily-quota.json
 */
export function createDailyQuotaTracker({ botDir, dailyLimit, timeZone = "Asia/Shanghai", getLimit } = {}) {
    const limit = Math.max(0, Math.floor(Number(dailyLimit) || 0));
    const tz = String(timeZone || "Asia/Shanghai").trim() || "Asia/Shanghai";
    const filePath = join(botDir, "daily-quota.json");
    /** @type {{ day: string, users: Record<string, number> }} */
    let data = { day: "", users: {} };
    try {
        if (existsSync(filePath)) {
            const raw = JSON.parse(readFileSync(filePath, "utf8")) || {};
            data = { day: String(raw.day || ""), users: raw.users && typeof raw.users === "object" ? raw.users : {} };
        }
    } catch {
        data = { day: "", users: {} };
    }
    ...
    function roll() {
        const d = today();
        if (data.day !== d) data = { day: d, users: {} };
    }

    function persist() {
        try {
            mkdirSync(dirname(filePath), { recursive: true });
            const entries = Object.entries(data.users).sort((a, b) => b[1] - a[1]).slice(0, 2000);
            data.users = Object.fromEntries(entries);
            writeFileSync(filePath, JSON.stringify({ day: data.day, tz, users: data.users }, null, 2));
        } catch {}
    }

    function capFor(userId) {
        if (typeof getLimit === "function") {
            const n = Math.floor(Number(getLimit(userId)) || 0);
            if (n > 0) return n;
        }
        return limit;
    }
    ...
    function check(userId) {
        ...
        roll();
        const used = Number(data.users[String(userId)] || 0) || 0;
        if (used >= cap) return { ok: false, used, limit: cap };
        return { ok: true, used, limit: cap };
    }

    function touch(userId) {
        if (capFor(userId) <= 0) return;
        roll();
        const id = String(userId);
        data.users[id] = (Number(data.users[id] || 0) || 0) + 1;
        persist();
    }

    return { check, touch, dailyLimit: limit, timeZone: tz };
}
