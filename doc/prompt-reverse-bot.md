# SecondaryBot（提示词反推 Bot）

专用无头 Bot：`@YourSecondaryBot`，只做**提示词反推**，可进群、群员可用。

## 配置

| 项 | 值 |
| :--- | :--- |
| Registry | `config/bots.json` → `SecondaryBot` |
...
| 菜单 | 仅 `/start` `/stop` |
| 模型策略 | `modelSet: "prompt-reverse"`；成员与默认模型只在 `config/models.json` 维护 |
...
2. 把 `@YourSecondaryBot` 拉进群（建议可发消息；不必管理员）

## 用法

推荐（群内）：

1. **@ 提及 + 发图**：`@YourSecondaryBot` 与图片同条（caption 可写补充说明），或先 @ 再发图带 caption
2. **纯图无字**：也可触发（反推专用放行）
3. **纯文字**：`@YourSecondaryBot` + 描述文字

- 冷却：同一用户约 **60s** 一次
- 模型由 `modelSets.prompt-reverse` 固定，无 `/model` 切换
...
日志：`bots/Headless/daemon.log`（进程级）；各 bot 状态：`bots/SecondaryBot/`。
